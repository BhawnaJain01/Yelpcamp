...
async function register(req, res, next) {
    try {
      const user = await campgrounds.findOne({ email: req.body.Email });
      if (user) {
  ...
  async function login(req, res, next) {
    ...
    try {
      const data = await campgrounds.find({ email: Email });
      ...
      const pass = await campgrounds.findOne({ email: Email });
